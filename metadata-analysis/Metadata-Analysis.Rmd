...
session_data <-
  read_csv(here("metadata-analysis/metadata/sessiondata.csv")) %>% # <- don't change this
  # ^ change where your file is in your project directory
  select("SessionID" = SessionToken,
         "Date" = Datestamp, SoftwareName)
...
combined_data <- full_join(staging, session_data, by = "SessionID", relationship = "many-to-many") %>%
  group_by(SessionID)
...
counts_software <- count(all_combined, WeekEnding, SoftwareName) %>%
  arrange(WeekEnding, desc(n))
